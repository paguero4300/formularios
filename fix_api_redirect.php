<?php
/**
 * Script para corregir el problema de redirección en las pruebas de API
...
<head>
    ...
    <title>Corrección de Redirección de API - Dr Security</title>
    <style>
        body {
...
<body>
    ...
    <h1>Corrección de Redirección de API</h1>
    
    <div class="info">
        <p>Este script corrige el problema de redirección en las pruebas de API.</p>
    </div>
    ...
    if (file_exists($testFile)) {
        ...
        if (strpos($content, 'function simulateApiRequest') !== false) {
            ...
            $pattern = "/\\\$url = 'http:\/\/' \. \\\$_SERVER\['HTTP_HOST'\] \. dirname\(\\\$_SERVER\['PHP_SELF'\]\) \. '\/api\/' \. \\\$endpoint;/";
            
            if (preg_match($pattern, $content)) {
                echo "<p class='success'>Se encontró la línea que construye la URL</p>";
                
                // Reemplazar la línea por una versión que use la URL absoluta
                ...
                // Añadir opción para seguir redirecciones
                ...
                if (file_put_contents($testFile, $newContent)) {
                    ...
                    echo "<p>Se ha modificado la construcción de la URL y añadido la opción para seguir redirecciones.</p>";
                } else {
                    echo "<p class='error'>No se pudo escribir en el archivo. Verifica los permisos.</p>";
                }
            } else {
                echo "<p class='warning'>No se encontró la línea exacta que construye la URL. Es posible que el formato haya cambiado.</p>";
            }
        } else {
