<?php
/**
 * Script para verificar que la API funciona correctamente con HTTPS
...
<head>
    ...
    <title>Verificación de API con HTTPS - Dr Security</title>
    <style>
        body {
...
<body>
    ...
    <h1>Verificación de API con HTTPS</h1>
    
    <div class="info">
        <p>Este script verifica que la API funciona correctamente con HTTPS y muestra información útil para la depuración.</p>
    </div>
    ...
    <table>
        ...
        <tr>
            <td>HTTPS</td>
            <td><?php echo isset($_SERVER['HTTPS']) ? $_SERVER['HTTPS'] : 'No definido'; ?></td>
        </tr>
        <tr>
    ...
    <h2>Prueba de URLs de API con HTTPS</h2>
    ...
    echo "<table>";
    echo "<tr><th>URL</th><th>Código HTTP</th><th>Respuesta</th><th>Contenido</th></tr>";
    
    foreach ($apiFiles as $apiFile) {
        $apiEndpoint = basename($apiFile);
        
        // Probar con la ruta HTTPS /api/
        $apiUrl = 'https://' . $_SERVER['HTTP_HOST'] . '/api/' . $apiEndpoint;
        ...
        if (!empty($result['response'])) {
            echo "<pre>" . htmlspecialchars(substr($result['response'], 0, 100)) . (strlen($result['response']) > 100 ? '...' : '') . "</pre>";
        } else {
            echo "<span class='warning'>Sin respuesta</span>";
        }
        echo "</td>";
        
        echo "</tr>";
        
        // Probar con la ruta HTTPS /formularios/api/
        $apiUrl = 'https://' . $_SERVER['HTTP_HOST'] . '/formularios/api/' . $apiEndpoint;
        $result = testApiUrl($apiUrl);
    ...
    <h2>Actualización de Scripts de Prueba para HTTPS</h2>
    
    <div class="info">
        <p>Para que las pruebas de API funcionen correctamente, es necesario actualizar los scripts para que usen HTTPS en lugar de HTTP.</p>
        <p>Aquí está el código que debe usarse en los scripts de prueba:</p>
...
function simulateApiRequest($endpoint, $method = 'GET', $data = null) {
    ...
    <p>Después de verificar que la API funciona correctamente con HTTPS, debes ejecutar los siguientes scripts:</p>
    ...
    <ol>
        ...
        <li><a href="fix_api_https.php">Corregir los scripts de API para usar HTTPS</a></li>
